...
namespace CrazyMinnow.SALSA.OneClicks
{
	public class OneClickCC4 : OneClickBase
	{
		/// <summary>
		/// RELEASE NOTES:
		/// 2025-09-25: Added error traps for emote frac changes and silence
		///			analyzer settings.
		///	2024-10-17: Updated SMR searches to include another facial hair option.
		...
		public static void Setup(GameObject gameObject)
		{
			...
			NewConfiguration(OneClickConfiguration.ConfigType.Salsa);
			{
				...
				AddSmrSearch("^(mustache|chin|sideburns|soul_patch|circle_thick_extracted[0-3]).*$");
			...
			NewConfiguration(OneClickConfiguration.ConfigType.Emoter);
			{
				...
				AddSmrSearch("^(brow|mustache|chin|sideburns|soul_patch|circle_thick).*$");
			...
			SetEmoteFrac("soften", 0.705f);
			SetEmoteFrac("browsUp", 0.469f);
			SetEmoteFrac("browUp", 0.817f);
			SetEmoteFrac("squint", 0.782f);
			SetEmoteFrac("focus", 1.0f);
			SetEmoteFrac("flare", 1.0f);
			SetEmoteFrac("scrunch", 0.853f);

			var silenceAnalyzer = selectedObject.GetComponent<SalsaAdvancedDynamicsSilenceAnalyzer>();
			if (silenceAnalyzer != null)
			{
		...
		private static void SetEmoteFrac(string emoteName, float frac)
		{
			EmoteExpression emote = null;
			emote = emoter.FindEmote(emoteName);
			if (emote != null)
				emote.frac = frac;
			else
				Debug.Log("Could not find emote: " + emoteName + " therefore, could not adjust emote frac settings.");
		}
	}
}
