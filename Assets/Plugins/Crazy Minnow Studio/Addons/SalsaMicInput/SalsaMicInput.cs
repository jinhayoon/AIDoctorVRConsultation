...
namespace CrazyMinnow.SALSA
{
	...
	public class SalsaMicInput : MonoBehaviour
	{
		...
		//	2.5.1:
		//		! Changing settings in prefabs now properly saves the setting.
		//	2.5.0.1:
		//		! StartMicrophone() now properly sets the selectedMic.
		//	2.5.0:
		//		NOTE: requires SALSA v2.5.0+
		//		+ basic co-routine startup protection -- there can be only one.
		//		+ preprocessor wraps for SALSA.
		//		+ basic OnEnable/Disable logic
		//		+ Updated to take advantage of SALSA 2.5.0 delegate plugin.
		//		~ General null-ref checks -- defensive coding.
		...
		{
			ConfigureAudioSourceForMicrophone(true);
			if (!isWiredUp)
				StartCoroWireUp();

			// Wire up the SALSA pointer delegate for Salsa polling of the record head pointer.
			#if CMS_SALSA
			var salsaInstance = GetComponent<Salsa>();
			if (salsaInstance)
			{
				if (linkWithSalsa)
				{
					salsaInstance.autoAdjustMicrophone = true;
					salsaInstance.clipHeadPointer = GetMicRecordHeadPointer;
				}
			}
			#endif
		}
		...
		private void OnDisable()
		{
			...
			if (isWiredUp)
			{
				ConfigureAudioSourceForMicrophone(false);
				StopMicrophone(selectedMic);
			}

			// Reset the SALSA pointer delegate for Salsa polling of the record head pointer.
			#if CMS_SALSA
			var salsaInstance = GetComponent<Salsa>();
			if (salsaInstance)
			{
				if (linkWithSalsa)
				{
					salsaInstance.autoAdjustMicrophone = false;
					salsaInstance.clipHeadPointer = null;
					salsaInstance.InitializeDelegates(); // repatch any null delegates.
				}
			}
			#endif
		}
		...
		private void ConfigureAudioSourceForMicrophone(bool configForMicrophone)
		{
			if (audioSrc == null)
				return;

			audioSrc.loop = configForMicrophone;
			audioSrc.mute = configForMicrophone;

			if (!configForMicrophone)
				audioSrc.Stop();
		}
		...
		public void StartMicrophone(string mic = null)
		{
			...
			if ( audioSrc != null )
			{
				if (!isWiredUp)
				{
					...
					{
						Debug.LogError("[StartMicrophone()] WARNING: cannot start, invalid sampleRate: " +
						               sampleRate.ToString() +
						               " The microphone may not be reporting a correct recording sample rate. Enable debug mode in micInput for details. It may be necessary to enable Override Rate in micInput if the compatible rates are not correctly reported by the microphone or Unity.");
						return;
					}
		...
		/// <summary>
		/// Used by SALSA as a delegate to fetch the microphone record head pointer.
		/// </summary>
		/// <returns>Current pointer</returns>
		public int GetMicRecordHeadPointer()
		{
			return Microphone.GetPosition(selectedMic);
		}
