...
public class PolarHeartRateService : IHeartService
{
    ...
    public bool isConnecting { get { return isScanningDevices || isScanningServices || isScanningCharacteristics; } }
    ...
    void Update()
    {
        ...
        if (isSubscribed)
        {
            ...
            while (HeartRateAPI.PollData(out res, false))
            {
                ...
                latestHeartData.heartRateBPM = (int)res.buf[1];
                bpm = $"Heart Rate: {res.buf[1].ToString()}, Average: {(float)(totalHeartRate / heartRateSamples)}";

                latestHeartData.heartRate_RR_Interval = res.buf[3] << 8 | res.buf[2];
                HR_RR_Interval = $"Heart Rate Interval: {latestHeartData.heartRate_RR_Interval}";
            }
        }
    }
